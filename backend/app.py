...
def addRoute():
    ...
    freeway = Freeway(name = route['name'], start_state = route['start_state'], end_state = route['end_state'], length = route['length'])
    freeway.insert()
    return jsonify( {
        'success' : True,
        'freeway' : freeway.format()
    }), 201
...
def addBusiness():
    ...
    business = BusinessEntity(name = req['name'], state = req['state'], city = req['city'],
                                zip = req['zip'], highways = req['highways'], type_of_business = req['type_of_business'],
                                miles_away = req['miles_away'], avg_amt = req['avg_amt'], lat = req['lat'], lng = req['lng'])
    freeway.business_entitys.append(business)
    freeway.insert()

    return jsonify({
        'success' : True,
        'business_entity' : business.format()
    }), 201
